...
`;const Ee=new Set(["unknown","unavailable",""]);let je=class extends ze{static getConfigElement(){return document.createElement("librus-device-editor")}static getStubConfig(){return{type:"custom:librus-grades-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 3}render(){if(!this._config||!this.hass)return W;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{deviceId:t,map:i}=e,s=this.hass,a=i.overall_average?s.states[i.overall_average]:void 0,r=_e(s,t,"subject_average").map(e=>({...e,state:s.states[e.entityId]})).filter(e=>e.state&&!Ee.has(e.state.state));if((!a||Ee.has(a.state))&&0===r.length)return this._message("mdi:school-outline",ke(s,"card.grades.empty"));const n=a&&!Ee.has(a.state)?Number(a.state):void 0,o=r.length?Math.max(...r.map(e=>Number(e.state.state))):6;return K`
  ...
  `}(u):W}
    ...
    `}};function rt(e){return"string"==typeof e?{value:e,allDay:e.length<=10}:e.date?{value:e.date,allDay:!0}:{value:e.dateTime??"",allDay:!1}}async function nt(e,t,i,s){const a=`calendars/${t}?start=${encodeURIComponent(i.toISOString())}&end=${encodeURIComponent(s.toISOString())}`,r=await e.callApi("GET",a);return Array.isArray(r)?r.map(e=>{const t=rt(e.start),i=rt(e.end);return{start:t.value,end:i.value,allDay:t.allDay,summary:e.summary??"",description:e.description,location:e.location}}):[]}at.styles=[Ce,Se,n`
      .type-icon {
        flex: none;
        width: 26px;
        height: 26px;
    ...
    `],e([ve()],lt.prototype,"_config",void 0),e([ve()],lt.prototype,"_events",void 0),lt=e([de("librus-free-days-card")],lt);const ut=[1,2,3,4,5];function ht(e){const t=new Date(e).getDay();return 0===t?7:t}let vt=class extends ze{constructor(){super(...arguments),this._events=[]}static getConfigElement(){return document.createElement("librus-device-editor")}static getStubConfig(){return{type:"custom:librus-week-timetable-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 4}connectedCallback(){super.connectedCallback(),this._refreshTimer=setInterval(()=>{this._fetch(!0)},18e5)}disconnectedCallback(){super.disconnectedCallback(),clearInterval(this._refreshTimer)}async _fetch(e=!1){if(!this.hass||!this._config)return;const t=this._resolveEntities();if("error"in t)return;const i=t.map.timetable;if(!i)return;const s=function(e){const t=new Date(e),i=ht(e.toISOString()),s=i>=6?8-i:1-i;return t.setDate(t.getDate()+s),t.setHours(0,0,0,0),t}(new Date),a=new Date(s);a.setDate(a.getDate()+5);const r=`${i}:${s.toDateString()}`;if(e||this._fetchedFor!==r){this._fetchedFor=r;try{this._events=await nt(this.hass,i,s,a)}catch{this._events=[]}}}render(){if(!this._config||!this.hass)return W;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const t=this.hass;if(this._fetch(),0===this._events.length)return this._message("mdi:calendar-week-outline",ke(t,"card.week_timetable.empty"));const i=[[],[],[],[],[]];for(const e of this._events){const t=ht(e.start);t>=1&&t<=5&&i[t-1].push(e)}i.forEach(e=>e.sort((e,t)=>e.start.localeCompare(t.start)));const s=Math.max(...i.map(e=>e.length),1),a=ut.map(e=>new Date(2026,0,e+4).toLocaleDateString(t.language,{weekday:"short"}));return K`
      <ha-card>
        <div class="header">
          ...
          <div class="title-block">
            ...
            <div class="subtitle">
              ${ke(t,function(e){return ht(e.toISOString())>=6}(new Date)?"card.week_timetable.subtitle_upcoming":"card.week_timetable.subtitle")}
            </div>
          </div>
        </div>
    ...
    `}};gt.styles=[Ce,Se],e([ve()],gt.prototype,"_config",void 0),gt=e([de("librus-school-card")],gt);const mt=new Set(["unknown","unavailable",""]);let pt=class extends ze{static getConfigElement(){return document.createElement("librus-device-editor")}static getStubConfig(){return{type:"custom:librus-today-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 2}connectedCallback(){super.connectedCallback(),this._tickTimer=setInterval(()=>this.requestUpdate(),6e4)}disconnectedCallback(){super.disconnectedCallback(),clearInterval(this._tickTimer)}render(){if(!this._config||!this.hass)return W;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,i=this.hass,s=e=>t[e]?i.states[t[e]]:void 0,a=s("lucky_number"),r=s("unread_messages"),n=s("unread_announcements"),o=s("timetable"),c=o?.attributes.message,d=o?.attributes.start_time,l="on"===o?.state;return K`
      <ha-card>
        ...
        <div class="stats">
          ${a&&!mt.has(a.state)&&!1!==a.attributes.is_today?K`<div class="stat"><div class="stat-value">${a.state}</div><div class="stat-label">${ke(i,"stat.lucky_number")}</div></div>`:W}
          ${r&&!mt.has(r.state)?K`<div class="stat"><div class="stat-value">${r.state}</div><div class="stat-label">${ke(i,"stat.unread_messages")}</div></div>`:W}
          ${n&&!mt.has(n.state)?K`<div class="stat"><div class="stat-value">${n.state}</div><div class="stat-label">${ke(i,"stat.new_announcements")}</div></div>`:W}
        </div>
        ${c&&d?K`
              <hr />
              <div class="list-item">
    ...
    `}};pt.styles=[Ce,Se],e([ve()],pt.prototype,"_config",void 0),pt=e([de("librus-today-card")],pt);const bt=new Set(["unknown","unavailable",""]);let ft=class extends ze{static getConfigElement(){return document.createElement("librus-device-editor")}static getStubConfig(){return{type:"custom:librus-week-summary-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 2}render(){if(!this._config||!this.hass)return W;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{deviceId:t,map:i}=e,s=this.hass,a=i.attendance?s.states[i.attendance]:void 0,r=i.behaviour_notices?s.states[i.behaviour_notices]:void 0,n=i.agenda?s.states[i.agenda]:void 0,o=new Date;o.setDate(o.getDate()-7);const c=o.toISOString().slice(0,10),d=_e(s,t,"subject_average").filter(e=>{const t=s.states[e.entityId]?.attributes.latest_grade_date;return t&&t>=c}).length,l=n?.attributes.message;return K`
      <ha-card>
        ...
        <div class="stats">
          ...
          ${a&&!bt.has(a.state)?K`<div class="stat ${Number(a.state)>0?"bad":""}"><div class="stat-value">${a.state}</div><div class="stat-label">${ke(s,"stat.absences")}</div></div>`:W}
          ${r&&!bt.has(r.state)?K`<div class="stat"><div class="stat-value">${r.state}</div><div class="stat-label">${ke(s,"card.behaviour_notices.title")}</div></div>`:W}
        </div>
        ${l?K`
              <hr />
              <div class="list-item">
    ...
    `}};ft.styles=[Ce,Se],e([ve()],ft.prototype,"_config",void 0),ft=e([de("librus-week-summary-card")],ft);const _t=new Set(["unknown","unavailable",""]);let yt=class extends ze{static getConfigElement(){return document.createElement("librus-device-editor")}static getStubConfig(){return{type:"custom:librus-lucky-number-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 2}render(){if(!this._config||!this.hass)return W;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,i=this.hass,s=t.lucky_number?i.states[t.lucky_number]:void 0;if(!s||_t.has(s.state))return this._message("mdi:dice-5-outline",ke(i,"empty.generic_error"));const a=s.attributes.is_today,r=s.attributes.day,n=!1===a&&r?ke(i,"card.lucky_number.subtitle_for_date",{date:De(r,i.language)}):ke(i,"card.lucky_number.subtitle");return K`
      <ha-card class="static">
        <div class="header">
