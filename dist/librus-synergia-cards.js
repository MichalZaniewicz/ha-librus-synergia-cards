...
`;function Oe(e){const t=Math.max(1,...e.map(e=>e.value));return K`
  ...
  `}function Le(e,t,i=64,s=6){const a=Math.max(0,Math.min(100,e)),r=(i-s)/2,n=2*Math.PI*r,o=i/2;return K`
  ...
  `}(s,{max:6})}</div>
    ...
    `:this._message("mdi:medal-outline",ze(i,"card.behaviour_grade.empty"))}};pt.styles=[Ae,Me,n`
      .grade-badge {
        font-size: 1.5rem;
        font-weight: 800;
        color: var(--lc-good);
  ...
  `}(v):V}
    ...
    `],e([pe()],Ct.prototype,"_config",void 0),Ct=e([ue("librus-attendance-heatmap-card")],Ct);const jt=[1,2,3,4,5],St=["excused","unexcused","late"],Dt={excused:"var(--lc-warn)",unexcused:"var(--lc-bad)",late:"var(--lc-brand)"},Et={excused:"stat.excused",unexcused:"stat.unexcused",late:"stat.late"};let It=class extends Ne{static getConfigElement(){return document.createElement("librus-device-editor")}static getStubConfig(){return{type:"custom:librus-attendance-weekday-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 3}render(){if(!this._config||!this.hass)return V;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,i=this.hass,s=t.attendance?i.states[t.attendance]:void 0,a=s?.attributes.by_weekday,r=jt.map(e=>{const t=a?.[String(e)];return(t?.excused??0)+(t?.unexcused??0)+(t?.late??0)}),n=r.reduce((e,t)=>e+t,0);if(!a||0===n)return this._message("mdi:chart-bar-stacked",ze(i,"card.attendance_weekday.empty"));const o=Math.max(...r,1),c={excused:0,unexcused:0,late:0};for(const e of jt){const t=a[String(e)];t&&(c.excused+=t.excused,c.unexcused+=t.unexcused,c.late+=t.late)}const d=jt.map(e=>new Date(2026,0,e+4).toLocaleDateString(i.language,{weekday:"short"}));return K`
      <ha-card>
        ...
        <div class="legend">
          ${St.map(e=>K`
              <span class="legend-item">
                <span class="dot" style="background:${Dt[e]}"></span>${ze(i,Et[e])} <b>${c[e]}</b>
              </span>
            `)}
        </div>
      </ha-card>
    ...
    `],e([pe()],At.prototype,"_config",void 0),At=e([ue("librus-behaviour-notices-tile-card")],At);const Ot={inbox:"recent",substitutions:"substitutions_recent",alerts:"alerts_recent",justifications:"justifications_recent"},Lt=[{key:"inbox",label:"mailbox.inbox"},{key:"notes",label:"mailbox.notes"},{key:"alerts",label:"mailbox.alerts"},{key:"substitutions",label:"mailbox.substitutions"},{key:"absences",label:"mailbox.absences"},{key:"justifications",label:"mailbox.justifications"},{key:"trash",label:"mailbox.trash"}];let Pt=class extends Ne{constructor(){super(...arguments),this._fullById={},this._pendingIds=new Set,this._errorIds=new Set}static getConfigElement(){return Ie()}static getStubConfig(){return{type:"custom:librus-messages-card"}}get _mailbox(){return this._config?.mailbox&&Ot[this._config.mailbox]?this._config.mailbox:"inbox"}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 3}async _onMessageClick(e){if(this._expandedId===e.id)return void(this._expandedId=void 0);if(this._expandedId=e.id,this._fullById[e.id]||this._pendingIds.has(e.id))return;const t=this._resolveEntities();if("error"in t||!this.hass)return;const i=e.mailbox??this._mailbox;this._pendingIds=new Set(this._pendingIds).add(e.id);const s=new Set(this._errorIds);s.delete(e.id),this._errorIds=s;try{const s=await Mt(this.hass,t.deviceId,e.id,i);this._fullById={...this._fullById,[e.id]:s}}catch{this._errorIds=new Set(this._errorIds).add(e.id)}finally{const t=new Set(this._pendingIds);t.delete(e.id),this._pendingIds=t}}_renderMessageBody(e){const t=this.hass;if(this._expandedId!==e.id)return K`<div class="item-text"><b>${e.topic}</b> - ${e.content}</div>`;const i=this._fullById[e.id];return i?K`
      ...
      `:this._errorIds.has(e.id)?K`<div class="item-text"><b>${e.topic}</b> - ${ze(t,"card.messages.fetch_failed")}</div>`:K`<div class="item-text"><b>${e.topic}</b> - ${ze(t,"empty.loading")}</div>`}render(){if(!this._config||!this.hass)return V;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,i=this.hass,s=t.unread_messages?i.states[t.unread_messages]:void 0;if(!s||"unavailable"===s.state)return this._message("mdi:email-outline",ze(i,"card.messages.unavailable"));const a=this._mailbox,r=s.attributes.mailbox_breakdown??{},n=s.attributes[Ot[a]]??[];Number(s.state);const o=this._config.max_items??6;return K`
      <ha-card>
        <div class="header">
    ...
    `}};$i.styles=[Ae,Me],e([pe()],$i.prototype,"_config",void 0),$i=e([ue("librus-week-summary-card")],$i);const ki=new Set(["unknown","unavailable",""]);let zi=class extends Ne{static getConfigElement(){return document.createElement("librus-device-editor")}static getStubConfig(){return{type:"custom:librus-lucky-number-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 2}render(){if(!this._config||!this.hass)return V;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,i=this.hass,s=t.lucky_number?i.states[t.lucky_number]:void 0;if(!s)return this._message("mdi:dice-5-outline",ze(i,"empty.generic_error"));if(ki.has(s.state))return this._message("mdi:dice-5-outline",ze(i,"card.lucky_number.empty"));const a=s.attributes.is_today,r=s.attributes.day,n=!1===a&&r?ze(i,"card.lucky_number.subtitle_for_date",{date:Fe(r,i.language)}):ze(i,"card.lucky_number.subtitle");return K`
      <ha-card
    ...
    `}};Di.styles=[Ae,Me],e([pe()],Di.prototype,"_config",void 0),Di=e([ue("librus-streak-card")],Di);const Ei=new Set(["unknown","unavailable",""]),Ii=["bronze","silver","gold","diamond"],Ti={bronze:0,silver:3,gold:4,diamond:5},Ni={bronze:"var(--lc-bronze)",silver:"var(--lc-silver)",gold:"var(--lc-amber)",diamond:"var(--lc-diamond)"},Ai={bronze:"bronze",silver:"silver",gold:"amber",diamond:"diamond"},Mi={bronze:"mdi:medal-outline",silver:"mdi:trophy-outline",gold:"mdi:trophy",diamond:"mdi:diamond-stone"};let Oi=class extends Ne{static getConfigElement(){return document.createElement("librus-device-editor")}static getStubConfig(){return{type:"custom:librus-rank-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 2}render(){if(!this._config||!this.hass)return V;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{map:t}=e,i=this.hass,s=t.rank?i.states[t.rank]:void 0;if(!s)return this._message("mdi:alert-circle-outline",ze(i,"empty.generic_error"));if(Ei.has(s.state)||(a=s.state,!Ii.includes(a)))return this._message("mdi:trophy-outline",ze(i,"card.rank.empty"));var a;const r=s.state,n=s.attributes.average,o=s.attributes.points_to_next_tier,c=Ii.indexOf(r),d=Ii[c+1],l=Ti[r],u=d?Ti[d]:void 0,h=void 0!==n&&void 0!==u?(n-l)/(u-l)*100:100;return K`
    ...
    `],e([pe()],Oi.prototype,"_config",void 0),Oi=e([ue("librus-rank-card")],Oi);let Li=class extends Ne{constructor(){super(...arguments),this._unlocked=[],this._storageKey=""}static getConfigElement(){return document.createElement("librus-device-editor")}static getStubConfig(){return{type:"custom:librus-achievements-card"}}setConfig(e){this._config=e,this._configuredDeviceId=e.device_id}getCardSize(){return 2}disconnectedCallback(){super.disconnectedCallback(),this._unsubscribe?.(),this._unsubscribe=void 0,this._subscribedDeviceId=void 0}_load(e){const t=`librus-achievements:${e}`;if(this._storageKey!==t){this._storageKey=t;try{const e=window.localStorage.getItem(t);this._unlocked=e?JSON.parse(e):[]}catch{this._unlocked=[]}}}_persist(){try{window.localStorage.setItem(this._storageKey,JSON.stringify(this._unlocked.slice(-50)))}catch{}}async _subscribe(e){if(this._subscribedDeviceId===e||!this.hass)return;this._subscribedDeviceId=e,this._unsubscribe?.(),this._unsubscribe=void 0;const t=this.hass.devices[e]?.config_entries??[];this._unsubscribe=await this.hass.connection.subscribeEvents(e=>{const i=e.data;t.length&&i.entry_id&&!t.includes(i.entry_id)||this._unlocked.some(e=>e.id===i.id)||(this._unlocked=[...this._unlocked,{id:i.id,title:i.title,when:(new Date).toISOString()}],this._persist())},"librus_synergia_achievement_unlocked")}render(){if(!this._config||!this.hass)return V;this._syncTheme();const e=this._resolveEntities();if("error"in e)return e.error;const{deviceId:t}=e,i=this.hass;if(this._load(t),this._subscribe(t),0===this._unlocked.length)return this._message("mdi:trophy-outline",ze(i,"card.achievements.empty"));const s=[...this._unlocked].sort((e,t)=>t.when.localeCompare(e.when));return K`
      <ha-card>
        <div class="header">
          ...
          <div class="title-block">
            ...
            <div class="subtitle">${ze(i,"card.achievements.count",{n:s.length})}</div>
          </div>
        </div>
        <div class="chips">
          ${s.map(e=>K`<span class="chip hot"><ha-icon icon="mdi:trophy-award"></ha-icon>${e.title}</span>`)}
        </div>
      </ha-card>
    `}};Li.styles=[Ae,Me,n`
      .chip.hot {
